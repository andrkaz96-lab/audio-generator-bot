...
_SPEECH_CONTRACT = (
    "КРИТИЧЕСКИЙ speech contract:\n"
    "- Нельзя молча удалять значимые токены: числа, годы, даты, время, проценты, дроби, валюты, температуры, диапазоны, версии, латиницу, бренды, модели, mixed-script фрагменты и цитаты.\n"
    "- Каждый такой токен должен либо сохраниться, либо быть преобразован в эквивалентную произносимую форму на русском без изменения фактического значения.\n"
    "- Не заменяй точные числа расплывчатыми словами вроде 'несколько', 'примерно', 'много', если в источнике есть точное значение.\n"
    "- Не переводи бренды и имена собственные на другой бренд или другое имя; допустима только безопасная verbalization.\n"
    "- Если сырая форма может плохо читаться русским TTS, перепиши ее в speech-safe форму. Примеры: '12.5%' → '12,5 процента', '1/4' → 'одна четверть', '12°C' → '12 градусов Цельсия', 'Wi‑Fi 6' → 'вай-фай 6', 'USB-C' → 'ю-эс-би си', 'GPT-4.1' → 'джи-пи-ти 4.1', 'OpenAI API' → 'OpenAI API' или эквивалентная произносимая форма без потери смысла.\n"
    "- Если в тексте есть английское слово внутри русского предложения, не выбрасывай его. Сохрани или аккуратно транслитерируй/вербализуй.\n"
)
...
SYSTEM_PROMPT_AUDIO_ADAPTED = (
    ...
    "11) Целевой объем: близко к close_to_source, обычно не менее 85% от содержательного объема исходного текста после очистки шума.\n"
    "12) После заголовка добавь короткое вступление (1–2 предложения), объясняющее тему статьи.\n"
    "13) В конце добавь короткий итог (1–2 предложения), подводящий итог основным мыслям.\n"
    "14) Few-shot ориентиры verbalization:\n"
    "- 'В 2025 году OpenAI показала рост на 12.5%.' → 'В 2025 году OpenAI показала рост на 12,5 процента.'\n"
    "- 'NVIDIA RTX 5090 и AI/ML-сервисы.' → 'NVIDIA RTX 5090 и AI/ML-сервисы' или эквивалентная произносимая форма без потери брендов, модели и аббревиатур.\n"
    "- 'Температура выросла до 12°C, скорость — 5 км/ч.' → 'Температура выросла до 12 градусов Цельсия, скорость — 5 километров в час.'\n"
    "- 'Версия GPT-4.1 доступна через API.' → 'Версия GPT-4.1 доступна через API' или эквивалентная произносимая форма без изменения версии и термина API.\n"
    "15) Верни только plain text в формате:\n"
    "заголовок\n\n"
    "вступление\n\n"
    "основной текст\n\n"
    "итог."
...
SYSTEM_PROMPT_AUDIO_SUMMARY = (
    ...
    "- избегай длинных вложенных конструкций и сложных письменных формулировок;\n"
    "- добавляй плавные переходы между абзацами, чтобы текст звучал как связный устный рассказ.\n"
    f"8) {_SPEECH_CONTRACT}"
    "9) Структура: после заголовка явно выдели три тезиса: 'Тезис 1', 'Тезис 2', 'Тезис 3', и для каждого дай 2-4 предложения пояснения.\n"
    "10) Few-shot ориентиры verbalization:\n"
    "- 'Компания потратила $20 млн и €15 млн.' → 'Компания потратила 20 миллионов долларов и 15 миллионов евро.'\n"
    "- 'Доля составила 1/4 рынка.' → 'Доля составила одну четверть рынка.'\n"
    "- 'Используется Wi‑Fi 6 и USB-C.' → 'Используется Wi‑Fi 6 и USB-C' или эквивалентная произносимая форма без потери стандарта и разъема.\n"
    "11) Верни только plain text: заголовок, пустая строка, затем итоговый текст."
    "12) Если исходный текст не на русском языке — переведи результат на русский язык. \n"
    "Имена собственные и названия компаний не переводить.\n"
)
